...
import { PyGADResult as PyGADResultType } from '../types';

interface PyGADResultProps {
    result: PyGADResultType | null;
    error: string | null;
}

const PyGADResult: React.FC<PyGADResultProps> = ({ result, error }) => {
    ...
    const handleDownload = () => {
        ...
        anchor.href = url;
        anchor.download = `pygad-result-${now}.json`;
        anchor.click();
    ...
    return (
        ...
        >
            ...
            {result.results && (
                <Box sx={{ mb: 3 }}>
                    ...
                    <Stack direction="row" flexWrap="wrap" gap={1} sx={{ mb: 1.5 }}>
                        <Chip label={`Function: ${result.results.function_name}`} variant="outlined" size="small" />
                        <Chip label={`Domain: [${result.results.domain?.join(', ')}]`} variant="outlined" size="small" />
                    </Stack>
                    <Box
                        sx={{
                    ...
                    >
                        <Typography variant="body1">
                            ...
                            {result.results.execution_time != null ? `${result.results.execution_time.toFixed(4)} s` : 'N/A'}
                        </Typography>
                        <Typography variant="body1">
            ...
            {/* ── Std Dev chart (required by P3 spec) ── */}
